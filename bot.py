...
def is_hubcdn_url(url):
    return "hubcdn" in url and "pixel" not in url

def is_pixel_hubcdn_url(url):
    return "pixel.hubcdn" in url

def is_vplink_url(url):
    return "vplink" in url or "short" in url

def is_gplinks_url(url):
    return "gplinks" in url or "get2.in" in url
...
def is_kmhd_link_url(url):
    return "links.kmhd.net" in url

def is_pikahd_url(url):
    return "pikahd" in url

def is_katdrama_url(url):
    return "katdrama" in url
...
def scrape_toonworld4all(url):
    ...
    try:
        ...
        for link in all_links:
            ...
            is_potential_link = any(k.lower() in text.lower() for k in keywords)
            ...
            if is_potential_link:
                ...
                if not href.startswith("#") and "javascript" not in href.lower():
                    if not any(d['link'] == href for d in download_links):
                        # Format text to include header for better readability in bot output
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for hit in hits:
            ...
            # Pack user_id and message_id into callback_data to verify ownership later
