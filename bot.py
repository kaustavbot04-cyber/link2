...
def get_user_stats(user_id):
    ...
    if user_id not in USER_STATS:
        ...
        # If the date stored is not today, reset the counters
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html(
        ...
        "4. <b>/usage:</b> Check your daily scrape limit."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_text.startswith("http"):
        ...
        if stats['scrape_count'] >= DAILY_LIMIT:
            await update.message.reply_html(
                f"⚠️ <b>Daily Limit Reached!</b>\n\nYou have used {stats['scrape_count']}/{DAILY_LIMIT} scrapes today.\n"
                "Please wait for tomorrow to scrape more links."
            )
            return
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check Limit for Button Click (which triggers scraping)
    stats = get_user_stats(user_id)
    if stats['scrape_count'] >= DAILY_LIMIT:
        await query.answer(text="⚠️ Daily Limit Reached! Use /usage to check.", show_alert=True)
        return
    ...
    # Increment usage counter since we proceed to scrape
    stats['scrape_count'] += 1
