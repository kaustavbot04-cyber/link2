...
# --- DOMAIN CHECKERS ---
def is_hubdrive_url(url): return "hubdrive" in url
def is_hubcloud_url(url): return "hubcloud" in url
def is_gofile_url(url): return "gofile" in url
def is_hubcdn_url(url): return "hubcdn" in url and "pixel" not in url
def is_pixel_hubcdn_url(url): return "pixel.hubcdn" in url or ("pixel" in url and "hubcdn" in url)
...
def is_howblogs_url(url): return "howblogs" in url
def is_4khdhub_url(url): return "4khdhub" in url
...
def is_kmhd_link_url(url): return "links.kmhd" in url
...
def is_extralink_url(url): return "extralink" in url
...
# --- NEW GOOGLE SEARCH FUNCTION FOR BUTTONS ---
        ...
        for g in soup.find_all('div', class_='g'):
            ...
            if anchors:
                ...
                if title:
                    ...
                    # ddjs: Check for typical file/movie download keywords in the title
                    if re.search(r'(480p|720p|1080p|download|movie|webrip|web-dl|hub|link)', title_text, re.IGNORECASE):
                        if link.startswith("http") and "google.com" not in link and len(results) < 5: # Limit to 5
                            # Extracting the cleaned link from the Google redirect URL
                            final_link = link
                            try:
                            ...
                            # Filter out links from the internal search sites to avoid duplication
                            if not any(site in final_link for site in ['hdhub4u', 'skymovieshd', 'cinevood']):
                                results.append((f"[G] {title_text}", final_link))
...
def bypass_gofile(url):
    ...
    # 2. Check for Direct Worker Link (Single File/Folder)
    # This worker link should directly resolve the file/folder or provide a listing.
    worker_url = f"https://gofile.dd-bypassed.workers.dev/{content_id}"
    
    try:
        worker_resp = requests.head(worker_url, headers=headers, allow_redirects=True, timeout=10)
        # Check if the HEAD request resolved to a known direct link
        if worker_resp.status_code == 200:
            final_url = worker_resp.url
            # If the final URL is not the worker itself and looks like a download/storage link
            if "workers.dev" not in final_url and final_url != worker_url:
                 # It's a direct file link (or it's redirecting to the file itself). Return it.
                 # For folders, it might still return the worker URL, so we rely on the redirect chain.
                 return final_url
        
    except Exception as e:
        logging.warning(f"GoFile Worker Direct Check Error: {e}")
        # Continue to API if worker direct check fails

    # 3. Use Official API for Folder Listing (Fallback/Structured Data)
    try:
        # Check if it's a folder using the list API endpoint from the prompt's instructions
        api_list_url = f"https://gofile.dd-bypassed.workers.dev/api/{content_id}"
        api_resp = requests.get(api_list_url, headers=headers, timeout=10)
        
        # If API returns a JSON structure, it's likely a folder.
        if api_resp.status_code == 200 and api_resp.headers.get('content-type') and 'json' in api_resp.headers['content-type']:
             try:
                 api_data = api_resp.json()
                 results = []
                 for item in api_data:
                     # Assuming the worker API returns a list of dicts with 'text' and 'link'
                     if item.get('link') and item.get('text'):
                         results.append({'text': item['text'], 'link': item['link']})
                 if results:
                    return results
             except json.JSONDecodeError:
                 logging.warning("GoFile Worker List API returned non-JSON/invalid data.")


        # 4. Fallback to Original GoFile API (for content information)
        account_resp = requests.post("https://api.gofile.io/accounts", headers=headers, timeout=10)
        if account_resp.status_code != 200: return worker_url # Return worker as fallback
        
        account_data = account_resp.json()
        if account_data['status'] != 'ok': return worker_url
        ...
        if content_resp.status_code == 200:
            ...
            if content_data['status'] == 'ok':
                if content_data['data']['type'] == 'file' and content_data['data'].get('link'):
                     # If it's confirmed to be a single file, return the worker link
                     return worker_url
                
                # Folder structure using the official API (if worker API failed)
        ...
        return worker_url # Final Fallback to worker link
    except Exception as e:
        logging.error(f"GoFile Error: {e}")
        return worker_url # Final Fallback
...
def bypass_hubdrive(url):
    ...
    parsed_url = urlparse(url)
    domain_origin = f"{parsed_url.scheme}://{parsed_url.netloc}"
    headers = {
        "User-Agent": USER_AGENT,
        "X-Requested-With": "XMLHttpRequest",
        "Referer": url,
    ...
    try:
        ...
        ajax_url = f"{domain_origin}/ajax.php?ajax=direct-download"
        data = {'id': file_id}
...
# --- SCRAPERS (Now returning Title + Links) ---
...
def scrape_hblinks(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True).replace("Download", "").strip() if soup.title else "HBLinks Content"
        links = []
...
def scrape_hdhub4u_page(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True).replace("Download", "").strip() if soup.title else "HDHub4u Movie"
        links = []
...
def scrape_skymovieshd(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "SkyMoviesHD Content"
        links = []
...
def scrape_4khdhub(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "4KHDHub Content"
        links = []
        ...
        for item in soup.find_all('div', class_='download-item'):
            ...
            if not header_text_div: continue
...
def scrape_filmyfiy(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "FilmyFiy Content"
        download_link = None
...
def scrape_vegamovies(url):
    ...
    try:
        response = requests.get(url, headers=headers, timeout=15)
        if response.status_code in [403, 503]: return "Vegamovies (Access Denied)", []

        response.raise_for_status()
        soup = get_soup(response.content)
        
        title = soup.title.get_text(strip=True).replace("Download", "").strip() if soup.title else "VegaMovies Content"
        links = []
...
def scrape_katmoviehd(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "KatMovieHD Content"
        links = []
        ...
        for a in soup.find_all('a', href=True):
            ...
            if '/tag/' in href or '/category/' in href: continue
...
def scrape_mymp4movies(url):
    ...
    try:
        ...
        if response.status_code in [403, 503]: return "Mymp4Movies (Access Denied)", []

        response.raise_for_status()
        soup = get_soup(response.content)
        
        title = soup.title.get_text(strip=True) if soup.title else "Mymp4Movies Content"
        links = []
        for a in soup.find_all('a', href=True):
            text = a.get_text().strip()
            href = a['href']
...
def scrape_kmhd_links(url):
    ...
    title = soup.title.get_text(strip=True) if soup.title else "KMHD Link"
    ...
    if "locked" in base_url:
        ...
        if form:
            ...
            headers = {'Referer': base_url, 'Origin': 'https://links.kmhd.net', 'Content-Type': 'application/x-www-form-urlencoded'}
            data = {}
    ...
    if upload_links_match:
        ...
        for key, value in res_ids:
            if value == "None" or value == "null": continue
...
def scrape_pixel_hubcdn(url):
    ...
    try:
        ...
        except: soup = BeautifulSoup(response.content, 'html.parser')

        title = soup.title.get_text(strip=True) if soup.title else "PixelHubCDN Content"
        links = []
...
def scrape_pikahd(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "Pikahd Content"
        links = []
        unique_hrefs = set()
        ...
        for a in soup.find_all('a', href=True):
            ...
            if not href or href.startswith('#') or href.startswith('javascript'): continue
            is_valid = False
            if any(x in href for x in ["links.kmhd.net", "hubcloud", "drive.google", "mega.nz"]): is_valid = True
            elif "Download" in text or "Links" in text:
                if href.startswith("http") and not any(x in href for x in ["category", "wp-json", "feed"]): is_valid = True
...
def scrape_katdrama(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "KatDrama Content"
        links = []
        ...
        for a_tag in soup.find_all('a', href=True):
            ...
            if "links.kmhd.net" in href and "/file/" in href:
                ...
                links.append({'text': link_text, 'link': href})
...
def scrape_toonworld4all(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "Toonworld4all Content"
        download_links = []
        ...
        for link in all_links:
            ...
            if any(k.lower() in text.lower() for k in keywords):
                ...
                if header: quality_header = header.get_text(strip=True)
...
def scrape_moviesmod(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "MoviesMod Content"
        download_links = []
...
def scrape_animeflix(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "AnimeFlix Content"
        links = []
...
def scrape_uhdmovies(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "UHDMovies Content"
            ...
            if container:
                for sibling in container.find_previous_siblings():
                    if sibling.name == 'p' and (sibling.find('strong') or sibling.find('b')):
                        ...
                        q_title = sibling.get_text(" ", strip=True)
                        break
            links.append({'text': q_title, 'link': link})
        return title, links
    except Exception as e:
...
def scrape_cinevood(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "Cinevood Content"
        links_data = []
        ...
        def is_valid_link(href):
            invalid_patterns = ['/1080p/', '/720p/', '/480p/', '/tg', 'facebook.com', 'twitter.com', 'whatsapp://', 'telegram.me/share']
            if any(p in href for p in invalid_patterns): return False
            return True

        for header in headers:
            current_quality = header.get_text().strip()
            if not current_quality: continue
            ...
            while sibling and sibling.name not in ['h5', 'h6']:
                if sibling.name in ['p', 'div']:
                    ...
                    for link in found_links:
                        ...
                        if not text: text = link.get('title', '').strip()
                        if not text: text = "Download Link"
                        if is_valid_link(href):
                            display_text = f"[{current_quality}] {text}"
                            links_data.append({'text': display_text, 'link': href})
                sibling = sibling.find_next_sibling()
        return title, links_data
    except Exception as e:
...
def scrape_extraflix(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "ExtraFlix Content"
        results = []
        for a in soup.find_all('a', href=lambda href: href and 'extralink' in href):
            link = a['href']
...
def scrape_extralink(url):
    ...
    try:
        ...
        title = "ExtraLink Collection" # Default
        links = []
...
def scrape_filepress(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "FilePress Content"
        links = []
        for a in soup.find_all('a', href=True):
            text = a.get_text(strip=True)
            href = a['href']
...
def scrape_hdwebmovies(url):
    ...
    try:
        ...
        title = soup.title.get_text(strip=True) if soup.title else "HDWebMovies Content"
        links_found = []
        for a in soup.find_all('a', href=True):
            text = a.get_text(strip=True)
            href = a['href']
...
def scrape_oxxfile(url):
    ...
    links = []
    title = "OxxFile Content"

    try:
        with sync_playwright() as p:
            browser = p.chromium.launch(
                headless=True,
                args=["--disable-blink-features=AutomationControlled", "--no-sandbox", "--disable-setuid-sandbox", "--disable-infobars", "--window-position=0,0", "--ignore-certifcate-errors", "--ignore-certifcate-errors-spki-list", "--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"]
            )
            ...
            if gdtot_btn.count() > 0:
                for i in range(3):
                    if "fetch failed" in page.content(): page.reload(); page.wait_for_timeout(5000); gdtot_btn = page.locator("button:has-text('Generate GDToT Link')")
                    ...
                    try:
                        ...
                        if "alibaba" in url_found or "google" in url_found: new_page.close(); page.wait_for_timeout(2000); continue
                        if url_found and url_found != "about:blank": links.append({'text': "GDToT", 'link': url_found}); new_page.close(); break
                        new_page.close()
                    except: page.wait_for_timeout(2000); continue
            ...
            if filepress_btn.count() > 0:
                for i in range(3):
                    try:
                        with context.expect_page(timeout=10000) as new_page_info:
                            try: filepress_btn.click()
                            except: pass
                        new_page = new_page_info.value
                        new_page.wait_for_load_state()
                        url_found = new_page.url
                        if "alibaba" in url_found or "google" in url_found: new_page.close(); page.wait_for_timeout(2000); continue
                        if url_found and url_found != "about:blank": links.append({'text': "FilePress", 'link': url_found}); new_page.close(); break
                        new_page.close()
                    except: page.wait_for_timeout(2000); continue
            browser.close()
    except Exception as e:
...
def scrape_watchadsontape(url):
    ...
    try:
        ...
        if match:
            domain_part = match.group(1); token_string = match.group(2); sub1 = int(match.group(3)); sub2 = int(match.group(4))
            final_link = f"https://{domain_part}{token_string[sub1:][sub2:]}"
        ...
        if match:
            domain_part = match.group(1); token_string = match.group(2); sub1 = int(match.group(3))
            final_link = f"https://{domain_part}{token_string[sub1:]}"
            return f"✅ <b>WatchAdsOnTape Bypassed!</b>\n\n🔗 <a href='{final_link}'>Download Link</a>"
...
def get_download_links(url):
    ...
    if is_gplinks_url(url):
        try:
            ...
            if result: return f"✅ <b>GPLinks Bypassed!</b>\n\n🔗 <a href='{result}'>Click to Download</a>"
            else: return "❌ Failed to bypass GPLinks."
        except Exception as e: return f"❌ Error processing GPLinks: {html.escape(str(e))}"

    if is_hubcloud_url(url):
        results = bypass_hubcloud(url)
        if results:
            msg = f"✅ <b>HubCloud Bypassed!</b>\n\n"
            for r in results: msg += f"📦 {html.escape(r['text'])}: {r['link']}\n"
            return msg
    ...
    if is_gofile_url(url):
        ...
        if isinstance(results, list):
            msg = f"✅ <b>GoFile Folder Extracted!</b>\n\n"
            for r in results: msg += f"📂 <a href='{r['link']}'>{html.escape(r['text'])}</a>\n"
            return msg
        elif isinstance(results, str):
            # If a string is returned, it's the direct worker link (for single files or as a final fallback)
            return f"✅ <b>GoFile Bypassed!</b>\n\n🔗 <a href='{results}'>Click to Download (Worker Direct)</a>"
    ...
    if is_hubcdn_url(url):
        ...
        if link: return f"✅ <b>HubCDN Bypassed!</b>\n\n🔗 <a href='{link}'>Click to Download</a>"
    ...
    if is_hubdrive_url(url):
        ...
        if isinstance(result, list):
            ...
            for r in result: msg += f"📦 {html.escape(r['text'])}: {r['link']}\n"
            return msg
        elif isinstance(result, str): return f"✅ <b>HubDrive Bypassed!</b>\n\n🔗 <a href='{result}'>Click to Download</a>"
        return "❌ Failed to bypass HubDrive link."
    
    # Note: bypass_vplink function definition is missing in the provided code.
    # Assuming it exists or its call should be commented out if not.
    ...
    if is_extralink_url(url):
        title, results = scrape_extralink(url)
        if results:
            msg = f"📂 <b>{html.escape(title)}</b>\n✅ <b>ExtraLink Expanded!</b>\n\n"
            for r in results: msg += f"🔗 {html.escape(r['text'])}: {r['link']}\n"
            return msg
    ...
    if "hblinks.dad" in url:
        page_title, links = scrape_hblinks(url)
    elif is_skymovieshd_url(url):
        page_title, links = scrape_skymovieshd(url)
    elif is_4khdhub_url(url):
        page_title, links = scrape_4khdhub(url)
    elif is_filmyfiy_url(url):
        page_title, links = scrape_filmyfiy(url)
    elif is_vegamovies_url(url):
        page_title, links = scrape_vegamovies(url)
    elif is_katmoviehd_url(url):
        page_title, links = scrape_katmoviehd(url)
    elif is_mymp4movies_url(url):
        page_title, links = scrape_mymp4movies(url)
    elif is_kmhd_link_url(url):
        page_title, links = scrape_kmhd_links(url)
    elif is_pixel_hubcdn_url(url):
        page_title, links = scrape_pixel_hubcdn(url)
    elif is_pikahd_url(url):
        page_title, links = scrape_pikahd(url)
    elif is_katdrama_url(url):
        page_title, links = scrape_katdrama(url)
    elif is_toonworld4all_url(url):
        page_title, links = scrape_toonworld4all(url)
    elif is_moviesmod_url(url):
        page_title, links = scrape_moviesmod(url)
    elif is_animeflix_url(url):
        page_title, links = scrape_animeflix(url)
    elif is_uhdmovies_url(url):
        page_title, links = scrape_uhdmovies(url)
    elif is_cinevood_url(url):
        page_title, links = scrape_cinevood(url)
    elif is_extraflix_url(url):
        page_title, links = scrape_extraflix(url)
    elif is_filepress_url(url):
        page_title, links = scrape_filepress(url)
    elif is_hdwebmovies_url(url):
        page_title, links = scrape_hdwebmovies(url)
    elif is_oxxfile_url(url):
        page_title, links = scrape_oxxfile(url)
    else:
        page_title, links = scrape_hdhub4u_page(url)

    if not links:
        return message + "⚠️ No download links found on the page."

    if page_title and page_title != "Unknown":
        message += f"📂 <b>{html.escape(page_title)}</b>\n"
    ...
    for item in links:
        ...
        if is_hubcloud_url(original):
            ...
            if isinstance(gf_results, list):
                ...
                message += "\n"
                continue
            elif isinstance(gf_results, str):
                bypassed = gf_results
                source_type = "GoFile Worker Direct"
            else:
                source_type = "GoFile (Manual/Failed)"
...
async def perform_google_search(query_text, update, context):
    try:
        search_query = f"{query_text} free download"
        url = f"https://www.google.com/search?q={urllib.parse.quote(search_query)}"
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
        ...
        if not results:
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"❌ No Google results found for '{html.escape(query_text)}'.")
            return

        msg = f"🌐 <b>Google Search Results for:</b> {html.escape(query_text)}\n\n"
        for title, link in results:
            msg += f"🎬 <a href='{link}'>{html.escape(title)}</a>\n\n"
            
        await context.bot.send_message(chat_id=update.effective_chat.id, text=msg, parse_mode='HTML', disable_web_page_preview=True)

    except Exception as e:
        logging.error(f"Google Search Error: {e}")
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Error performing Google search.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        google_hits_future = loop.run_in_executor(None, search_google, user_text) # DDJS - Added Google Search for links
        
        hdhub_hits = await hdhub_hits_future
        skymovies_hits = await skymovies_hits_future
        cine_hits = await cine_hits_future
        google_hits = await google_hits_future # DDJS - Get Google Hits
        
        await context.bot.delete_message(chat_id=update.effective_chat.id, message_id=status_msg.message_id)

        if not hdhub_hits and not skymovies_hits and not cine_hits and not google_hits:
            # Fallback to Google Search button even if internal scrapers fail? 
            # Or just say no results. Let's provide the Google button regardless.
            pass
        ...
        # DDJS - Process Google Search hits
        for title, link in google_hits:
            search_id = str(uuid.uuid4())[:8]
            SEARCH_CACHE[search_id] = link
            callback_data = f"{search_id}|{user_id}|{message_id}"
            
            display_title = (title[:30] + '..') if len(title) > 30 else title
            keyboard.append([InlineKeyboardButton(f"🌐 {display_title}", callback_data=callback_data)])
        # ---------------------------------------------
        ...
        # --- ADD GOOGLE SEARCH BUTTON (As fallback/alternative listing) ---
        search_id_google = str(uuid.uuid4())[:8]
        SEARCH_CACHE[search_id_google] = {'type': 'google_search', 'query': user_text}
        callback_data_google = f"google|{user_id}|{search_id_google}"
        keyboard.append([InlineKeyboardButton("🌐 Full Google Search", callback_data=callback_data_google)])
        # ----------------------------------------------

        reply_markup = InlineKeyboardMarkup(keyboard)
        total_hits = len(hdhub_hits) + len(skymovies_hits) + len(cine_hits) + len(google_hits)
        
        if total_hits == 0:
             await update.message.reply_text(f"❌ No internal results found for '{html.escape(user_text)}'.\nTry Full Google Search:", reply_markup=reply_markup, parse_mode='HTML')
        else:
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check for Google Search callback
    if query.data.startswith("google|"):
        data_parts = query.data.split('|')
        if len(data_parts) == 3:
            _, owner_id, search_id = data_parts
            if int(owner_id) != query.from_user.id:
                await query.answer(text="This is not your search request...", show_alert=True)
                return
            
            cached_data = SEARCH_CACHE.get(search_id)
            if cached_data and isinstance(cached_data, dict) and cached_data.get('type') == 'google_search':
                user_query = cached_data.get('query')
                await query.answer()
                await query.edit_message_text(f"🌐 Searching Google for '{html.escape(user_query)}'...")
                await perform_google_search(user_query, update, context)
                return
            else:
                await query.answer(text="Search expired.", show_alert=True)
                return

    # Check for Standard Search callback
    data_parts = query.data.split('|')
    ...
    if not url or isinstance(url, dict): # Handle expired or incorrect type
