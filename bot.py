...
def is_cinevood_url(url): return "cinevood" in url
...
def scrape_cinevood(url):
    ...
    try:
        ...
        for a in soup.find_all('a', href=True):
            ...
            text_lower = text.lower()
            if any(q in text_lower for q in ['480p', '720p', '1080p', '2160p', '4k']) or \
               (any(k in text_lower for k in ['download', 'links', 'gdrive', 'mega']) and len(text) < 50):
                links.append({'text': text, 'link': href})
        return links
    except Exception as e:
        logging.error(f"Cinevood Error: {e}")
        return []
